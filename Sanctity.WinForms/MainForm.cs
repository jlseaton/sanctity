...
namespace Sanctity.WinForms
{
    public partial class MainForm : Form
    {
        ...
        string[] playerPics = { "barbarian", "fighter1", "thief", "fighter2", "wizardress", "monk", "cleric" };
        ...
        private void RefreshStatus()
        {
            ...
            {
                ShowImage(playerPics[this.comboBoxPlayers.SelectedIndex], this.pictureBoxPC);
                ...
                else
                {
                    ...
                    if (packet.Tile != null)
                    {
                        ...
                        if (Config.Graphics)
                        {
                            ...
                            {
                                ShowCenterImage(pictureBoxNPC, "stairsup.bmp");
                            }

                            if (Tile.Down > 0 || Tile.Down < 0)
                            {
                                ShowCenterImage(pictureBoxNPC, "stairsdown.bmp");
                            }
                        }
                        ...
                        if (packet.NPCs != null && packet.NPCs.Any())
                        {
                            ...
                            foreach (var npc in packet.NPCs)
                            {
                                if (Config.Graphics)
                                {
                                    ShowImage(npc.Value.Name, this.pictureBoxNPC);
                                }
        ...
        private void ShowImage(string imageName, PictureBox box)
        {
            var fullName = "Images\\" + imageName.ToLower();

            if (File.Exists(fullName + ".bmp"))
            {
                fullName = imageName + ".bmp";
            }
            else if (File.Exists(fullName + ".png"))
            {
                fullName = imageName + ".png";
            }
            else if (File.Exists(fullName + ".jpg"))
            {
                fullName = imageName + ".jpg";
            }
            else
            {
                return;
            ...
            ShowCenterImage(box, fullName);
        }

        private void ShowCenterImage(PictureBox picBox, string fileName)
        {
            if (!Config.Images)
                return;
